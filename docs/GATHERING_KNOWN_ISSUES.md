...
## 中：M3 — 跨隊掃碼用 limit(1) 抽樣判斷大隊歸屬，SquadName 不一致時誤判

- **位置**：squad-gathering.ts L625-633。大隊長掃非本小隊的場時，判斷「是否同大隊」是從目標小隊**隨機抓一名**有 SquadName 的成員（`.not('SquadName','is',null).limit(1)`，無 order）當作該小隊的大隊歸屬。
- **觸發**：目標小隊成員的 SquadName 資料不一致（有人填錯、有人 null）→ `limit(1)` 抓到哪筆不確定 → 可能誤判大隊長有/無權限。另：若整隊 SquadName 全為 null → 一律拒絕大隊長掃碼。
- **後果**：跨隊掃碼權限判斷不穩定（偶發放行/誤擋）。
- **修法方向**：以 session 自身應落定的 squad 歸屬，或建立 team→squad 的權威對照表來判斷，而非從成員抽樣。
- **嚴重度**：中（權限正確性，依賴資料品質）。
...
1. **M3**：跨隊掃碼權限誤判，影響實際運作，建議優先。
2. **H4**：金額正確性，但觸發苛刻；需 DB 層改動，獨立排程。
3. **L2-L5**：低風險，可併入日常維護或忽略（L4/L5 先確認是否仍適用）。
