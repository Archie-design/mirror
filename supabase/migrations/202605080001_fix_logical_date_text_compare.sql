-- 修補 process_checkin 中邏輯日期文字比對失效的 bug
--
-- 問題：
--   `(date(...) - INTERVAL '1 day')::text` 在 PG 中回傳 'YYYY-MM-DD HH:MI:SS'
--   （因為 date - interval 回傳 timestamp，不是 date），
--   導致與 p_logical_today（YYYY-MM-DD）字串比對永遠不相等。
--
-- 影響：
--   1. p1_dawn 前置檢查找不到「凌晨完成的 p1」→ 誤判「需先完成打拳」
--   2. 凌晨完成的 d1–d8 / p1–p5 / 飲控 dedup 與 cap 檢查失效
--      （理論上學員可重複打卡同一項或超過每日 3 項上限）
--
-- 修法：
--   將所有 `- INTERVAL '1 day'` 改為 `- 1`（integer），
--   讓 PG 回傳 date 而非 timestamp，::text 即得 'YYYY-MM-DD'。
