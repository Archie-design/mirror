-- 修復 p1_dawn 跨午邏輯日期邊界導致「需先完成打拳」錯誤
--
-- 根本原因：
--   用戶在 11:55 AM 完成 p1（邏輯日 = 昨天），UI 尚未刷新仍顯示破曉按鈕可按。
--   用戶在 12:05 PM 按破曉打拳，此時 p_logical_today = 今天。
--   RPC 只找邏輯日 = 今天 的 p1，找不到 → 誤報「需先完成打拳」。
--
-- 修法：
--   p1_dawn 的前置檢查改為：p1 在 p_logical_today 或 p_logical_today - 1 天 均可。
--   最大容許窗口為 36 小時（午前完成 p1 → 午後立即記錄 p1_dawn）。
--   正常情境不受影響；過舊（2 天前）的 p1 仍無法解鎖 p1_dawn。
