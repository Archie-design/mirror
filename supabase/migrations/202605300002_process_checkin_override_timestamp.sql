-- process_checkin 加入可選的時間戳覆蓋參數
--
-- 背景：
--   reviewGathering / adminBackfillGathering 核准時呼叫 process_checkin，
--   Timestamp 固定寫入 NOW()（核准時刻）。若 5/24 凝聚在 5/26 補報，
--   DailyLog.Timestamp = 5/26（W3），導致 5/29 凝聚的 W3 週上限被誤佔。
--
-- 修法：
--   新增 p_override_timestamp TIMESTAMPTZ DEFAULT NULL。
--   有帶值時 INSERT 使用該時間戳（例如 gathering_date 正午 TW），
--   否則維持 NOW() 行為，向下相容所有現有呼叫。
