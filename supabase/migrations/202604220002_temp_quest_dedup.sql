-- 為臨時加碼任務（temp_TIMESTAMP|YYYY-MM-DD）新增 server 端每日重複檢查
-- 背景：
--   規格 §4.4 明文「預設每位學員每日最多 1 次」
--   原 process_checkin RPC 對 temp_% 走到「其他任務...無每日上限，由呼叫端處理」分支
--   前端雖有 isMax 判斷，但任何人從 DevTools 直接呼叫 RPC 即可繞過
--   DailyLogs 亦無 (UserID, QuestID) unique constraint，可以同一 questId 重複入帳
-- 修補：RPC 前段加 ELSIF p_quest_id LIKE 'temp\_%' 分支，檢查 DailyLogs 是否已有該精確 questId
...
BEGIN
  ...
  RETURN jsonb_build_object('success', true, 'newScore', v_new_score);
END;
$$;
