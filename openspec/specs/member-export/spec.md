# 成員清單匯出（member-export）

## Purpose

定義管理員後台與大隊長總部的「下載成員清單」功能：scope 判定、CSV 雙區塊結構、欄位內容、效能上限。

> 來源：[openspec/changes/member-export-with-team-summary](../../changes/archive/2026-05-02-member-export-with-team-summary/)
...
1. **成員區**：表頭為 `UserID,姓名,大隊,小隊,角色,累積積分,連勤天數,最後打卡日`，每行一個成員。
2. **小隊摘要區**：表頭為 `小隊,隊員數,總積分,平均積分,隊長,已核准凝聚次數`，每行一個小隊。
...
- **連勤天數**：`CharacterStats.Streak`
- **最後打卡日**：`CharacterStats.LastCheckIn`，缺值時填空字串
...
- **THEN** 系統用標準 RFC 4180 規則跳脫（雙引號加倍、整個欄位包雙引號）
...
- **小隊**：`TeamName`
- **隊員數**：該小隊內成員總數（含小隊長）
- **總積分**：該小隊所有成員 `Score` 加總
- **平均積分**：該小隊所有成員 `Score` 的算術平均，**取整數**（floor），無成員時填 0
- **隊長**：該小隊內 `IsCaptain=true` 成員的 `Name`，多人時以「、」串接，無時填「（未設定）」
- **已核准凝聚次數**：該小隊在 `SquadGatheringSessions` 中 `status='approved'` 的記錄數

#### Scenario: 小隊摘要計算

- **WHEN** 第一大隊-第1小隊有 6 名成員，分數 [100, 200, 150, 50, 300, 0]
- **THEN** 「總積分」=800、「平均積分」=133（800/6 floor）、「隊員數」=6
...
- **管理員後台**：`components/Admin/AdminDashboard.tsx` 「I.i 學員名冊管理」區塊內，與「批量匯入名冊」並列。
- **大隊長總部**：`components/Tabs/CommandantTab.tsx` 首屏（進入頁時即可見），不需要捲動。
...
- **WHEN** 管理員點按鈕
- **THEN** 瀏覽器下載 `members_export_<日期>.csv`
